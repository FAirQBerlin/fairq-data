...
-- fairq_features.cams_all source

CREATE VIEW fairq_features.cams_all
(
...
FROM fairq_features.cams_old_rescaled AS cor
LEFT JOIN fairq_features.coord_mapping_cams AS cmc ON (cmc.cams_old_lat_int = cor.lat_int) AND (cmc.cams_old_lon_int = cor.lon_int)
UNION ALL
SELECT
    toDateTime(date_time,
 'Europe/Berlin') AS date_time,
...
-- fairq_features.coord_mapping_cams source

CREATE VIEW fairq_features.coord_mapping_cams
(
...
-- fairq_features.coord_mapping_dwd_det source

CREATE VIEW fairq_features.coord_mapping_dwd_det
(
...
WITH
    ...
    (
        ...
        INNER JOIN fairq_features.mapping_reprojection USING (lat_int,
 lon_int)
    ),
    ...
    (
        SELECT DISTINCT
            ...
            lon_int / 100000 AS lon
        FROM fairq_features.dwd_observations_filled
        INNER JOIN fairq_features.mapping_reprojection USING (x,
 y)
    ),
...
GROUP BY
   ...
   -- fairq_features.date_time source

CREATE VIEW fairq_features.date_time
(
...
WITH days_per_year AS
    (
        ...
        FROM fairq_features.features_date_time
...
-- fairq_features.emissions source

CREATE VIEW fairq_features.emissions
(
...
SELECT
    ...
    pm25_ge15
FROM fairq_features.coord_mapping_emissions_stadt
LEFT JOIN fairq_raw.stadtstruktur_emissions USING (idnr_1km);

-- fairq_features.features_date_time source

CREATE VIEW fairq_features.features_date_time
(
...
-- fairq_features.latest_cams source

CREATE VIEW fairq_features.latest_cams
(
...
-- fairq_features.traffic_det source

CREATE VIEW fairq_features.traffic_det
(
...
INNER JOIN fairq_features.mapping_reprojection AS mr ON (tdcsp.lat_int = mr.lat_int) AND (tdcsp.lon_int = mr.lon_int))
COMMENT 'code to create this view can be found at https://github.com/INWT/fairq-gap-filling/inst/db/schema.sql';
