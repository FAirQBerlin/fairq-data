-- insert into  fairq_features.statdstruktur_features
insert into fairq_features.statdstruktur_features (*)
select 
  berlin.x as x,
  berlin.y as y,
...
from
  fairq_features.coord_stadt_berlin as berlin
left join
  fairq_features.land_use AS land
  on (berlin.x = land.x) and (berlin.y = land.y)
left join
  fairq_features.emissions AS emi
  ON (land.x = emi.x) AND (land.y  = emi.y)
left join
  fairq_features.traffic_volume AS traffic_vol
  ON (land.x = traffic_vol.x) AND (land.y  = traffic_vol.y)
left join
  fairq_features.traffic_intensity AS tint
  ON (land.x = tint.x) AND (land.y  = tint.y)
left join
  fairq_features.buildings
  ON (land.x = buildings.x) AND (land.y  = buildings.y);

-- insert into coord_grid_batches
insert into fairq_features.coords_grid_batches (*)
with rn as (
select
  id,
  x,
...
insert into fairq_features.coords_grid_sim_batches (*)
select 
  stadt_x x, 
  stadt_y y, 
  element_nr, 
...
from 
  fairq_features.coord_mapping_stadt_streets
inner join
  fairq_raw.stadtstruktur_network_streets using(element_nr)
where 
element_nr in ('38550024_38550010.02', '45510028_45520013.01');

insert into fairq_features.coord_mapping_stadt_passive 
with coords_passive_samplers as (
select 
 id, 
 toInt32(100000 * lat) lat_int, 
 toInt32(100000 * lon) lon_int
from 
  fairq_raw.stadtstruktur_measuring_stations
where 
  messnetz in ('Passivsammler', 'RUBIS + Passivsammler')
and stand = 'aktuell'
),
dists as (
 select
   id,
   coords_passive_samplers.lat_int as passive_lat,
   coords_passive_samplers.lon_int as passive_lon,
   x, 
   y,
   coord_mapping_stadt_reprojection.lat_int as stadt_lat,
   coord_mapping_stadt_reprojection.lon_int as stadt_lon,
   L2Distance((passive_lat, passive_lon), 
   (coord_mapping_stadt_reprojection.lat_int, coord_mapping_stadt_reprojection.lon_int)) AS dist
from
 coords_passive_samplers
CROSS JOIN
 fairq_features.coord_mapping_stadt_reprojection
 )
select
 id,
 argMin((x, y), dist).1 as stadt_x,
 argMin((x, y), dist).2 as stadt_y
from
 dists
group by id;

insert into fairq_features.stations_for_predictions (station_id, id, is_active)
...
insert into fairq_features.scaling_kfz_station
with kkfz as (
select 
  distinct round(kfz_per_24h / 1000) kkfz_per_24h
from 
  fairq_features.traffic_model_scaling
...
insert into fairq_features.mapping_stations_dwd
with coords_dwd as (
select distinct 
 x, 
 y,
...
from 
 fairq_features.dwd_observations_filled
inner join 
 fairq_features.mapping_reprojection using(x, y)
 ),
coords_stations as (
select distinct 
 station_id, 
 ...
 lon_int / 100000 as lon
from 
 fairq_features.messstationen_filled
inner join 
 fairq_features.mapping_reprojection using(x, y)
),
dists as (
...
insert into fairq_features.coord_stadt_all
select distinct x, y from fairq_features.streets;

insert into fairq_features.coord_mapping_stadt_station
...
insert into  fairq_features.coord_mapping_stations_cams
with coords_cams as (
select distinct 
 x, 
 y
from 
 fairq_features.latest_cams
 ),
coords_stations as (
select distinct 
 station_id, 
...
insert into fairq_features.coord_mapping_stadt_det
with coords_det as (
select distinct
...
inner join
  fairq_features.mapping_reprojection using(lat_int, lon_int)
),
 dists as (
 select
   mq_name,
...
from
 coords_det
CROSS JOIN
 fairq_features.coord_stadt_all coords_stadt
 )
select
 mq_name,
 det_x,
...
insert
	into
	fairq_features.coord_mapping_stadt_dwd
