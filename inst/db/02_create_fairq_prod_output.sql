...
create database if not exists fairq_prod_output;


-- fairq_prod_output.model_description_old definition

CREATE TABLE fairq_prod_output.model_description_old
(
...
-- fairq_prod_output.model_predictions_grid definition

CREATE TABLE fairq_prod_output.model_predictions_grid
(
...
ALTER TABLE fairq_prod_output.model_predictions_grid
    MODIFY TTL date_time + INTERVAL 3 MONTH DELETE;

ALTER TABLE fairq_prod_output.model_predictions_grid
modify column value Decimal(4,1);

-- fairq_prod_output.model_predictions_spatial_cv definition

CREATE TABLE fairq_prod_output.model_predictions_spatial_cv
(
...
-- fairq_prod_output.model_predictions_stations definition

CREATE TABLE fairq_prod_output.model_predictions_stations
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time_forecast` DateTime('Europe/Berlin') COMMENT 'timestamp when the prection was made',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp the prediction was made for',
...
-- fairq_prod_output.model_predictions_temporal definition

CREATE TABLE fairq_prod_output.model_predictions_temporal
(
...
-- fairq_prod_output.model_predictions_temporal_cv definition

CREATE TABLE fairq_prod_output.model_predictions_temporal_cv
(
...
-- fairq_prod_output.models_final definition

CREATE TABLE fairq_prod_output.models_final
(
...
-- fairq_prod_output.model_description definition
CREATE TABLE fairq_prod_output.model_description
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time_training_execution` DateTime COMMENT 'when the model was developed',
    `pollutant` String COMMENT 'no2,\n pm10 or pm 2.5',
...
CREATE TABLE fairq_prod_output.traffic_model_description
(
...
-- fairq_prod_output.traffic_model_predictions_temporal_cv definition

CREATE TABLE fairq_prod_output.traffic_model_predictions_temporal_cv
(
...
-- fairq_prod_output.updated_table definition

CREATE TABLE fairq_prod_output.updated_table
(
...
create table fairq_prod_output.api_grid 
(
    `x` Int32,
    `y` Int32,
    `id` Int,
...
CREATE TABLE fairq_prod_output.preds_grid_filled_with_null
(
    `model_id` UInt16,
...
CREATE TABLE fairq_prod_output.model_predictions_grid_latest
(
    `model_id` UInt16,
    `date_time` DateTime,
    `x` UInt32,
...
create table fairq_prod_output.api_streets
(
    `element_nr` String,
    `date_time_forecast` DateTime('Europe/Berlin'),
    `first_pred_date_time` DateTime('Europe/Berlin'),
...
CREATE TABLE fairq_prod_output.model_predictions_stations_latest
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `station_id` String COMMENT 'id of measuring station',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp the prediction was made for',
...
CREATE TABLE fairq_prod_output.pred_stations_filled_with_null
(
    `model_id` UInt16,
    `date_time` DateTime('Europe/Berlin'),
    `station_id` String,
...
CREATE TABLE fairq_prod_output.api_stations
(
    `station_id` String,
    `x` Int32,
    `y` Int32,
...
CREATE TABLE fairq_prod_output.model_predictions_thresholds
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time_forecast` DateTime('Europe/Berlin') COMMENT 'timestamp when the prection was made',
...
CREATE TABLE fairq_prod_output.api_simulation
(
    `element_nr` String,
    `date_time_forecast` DateTime('Europe/Berlin'),
...
-- fairq_prod_output.cap_values definition

CREATE TABLE fairq_prod_output.cap_values
(
...
-- api lor
CREATE TABLE fairq_prod_output.api_lor
(
    `PLR_ID` String,
    `date_time_forecast` DateTime('Europe/Berlin'),
    `first_pred_date_time` Date,
...
CREATE TABLE fairq_prod_output.model_predictions_grid_history 
(
...
CREATE TABLE fairq_prod_output.model_predictions_stations_lags
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time_forecast` DateTime('Europe/Berlin') COMMENT 'timestamp when the prection was made',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp the prediction was made for',
...
CREATE TABLE fairq_prod_output.selected_times_coords_ids
(
    `date_time` DateTime('Europe/Berlin'),
    `x` Int32,
    `y` Int32,
...
CREATE TABLE fairq_prod_output.preds_grid_sim_filled_with_null
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp when the prection was made',
...
CREATE TABLE fairq_prod_output.model_predictions_grid_sim
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time_forecast` DateTime('Europe/Berlin') COMMENT 'timestamp when the prection was made',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp the prediction was made for',
...
CREATE TABLE fairq_prod_output.model_predictions_grid_sim_latest
(
    `model_id` UInt16 COMMENT 'unique model id; applies to only one pollutant',
    `date_time` DateTime('Europe/Berlin') COMMENT 'timestamp the prediction was made for',
    `x` UInt32 COMMENT 'x coordinate of prediction',
...
CREATE TABLE fairq_prod_output.selected_times_coords_ids_pct
(
    `date_time` DateTime('Europe/Berlin'),
    `model_id` UInt16,
    `x` Int32,
